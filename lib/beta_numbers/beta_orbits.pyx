...
def calc_orbits(
    ...
    with timers.time("calc_orbits callee"):
        ...
        if proc_index == 0:
            ...
            log('`calc_orbits` calling `_update_status_reg_apos`')
        ...
        with timers.time_cm(
            ...
            stack(
                ...
                periodic_reg.open(), status_reg.open(), coef_orbit_reg_highprec.open(True),
                periodic_reg_highprec.open(True)
            )
        ) as (
            perron_polys_reg, perron_nums_reg, poly_orbit_reg, coef_orbit_reg, periodic_reg, status_reg,
            coef_orbit_reg_highprec, periodic_reg_highprec
        ):

            for poly_apri in timers.time_iter("poly_apri loop", perron_polys_reg):

                log(f'`calc_orbits` poly_apri loop {poly_apri}')
                                        ...
                                        with timers.time_cm(
                                            ...
                                            for index in incomplete_indices:
                                                ...
                                                _single_orbit(
                                                    ...
                                                    -1,
                                                    -1,
                                                    coef_orbit_reg_highprec,
                                                    periodic_reg_highprec
                                                )
