...
from beta_numbers.perron_numbers import calc_salem_nums, calc_salem_nums_setup_regs

def f(num_procs, proc_index, salem_polys_reg, salem_nums_reg, salem_conjs_reg, max_sum_abs_coef, blk_size, dps, timers, debug_dir):

    set_dir(debug_dir)
    calc_salem_nums(
        max_sum_abs_coef, blk_size, dps, salem_polys_reg, salem_nums_reg, salem_conjs_reg, num_procs, proc_index, timers
    )
...
if __name__ == "__main__":
    ...
    if do_setup:
        ...
        salem_polys_reg, salem_nums_reg, salem_conjs_reg = calc_salem_nums_setup_regs(dir_)

    else:

        salem_polys_reg = load('salem_polys_reg', dir_)
        salem_nums_reg = load('salem_nums_reg', dir_)
        salem_conjs_reg = load('salem_conjs_reg', dir_)

    timers = Timers()
    parallelize(
        num_procs, f,
        (salem_polys_reg, salem_nums_reg, salem_conjs_reg, max_sum_abs_coef, blk_size, dps, timers, debug_dir),
