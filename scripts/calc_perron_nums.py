...
def f(num_procs, proc_index, perron_polys_reg, perron_nums_reg, perron_conjs_reg, max_sum_abs_coef, blk_size, timers, debug_dir):
...
if __name__ == "__main__":
    ...
    timeout = int(slurm_timecode_to_timedelta(sys.argv[5]).total_seconds() * 0.90)
    update_period = int(sys.argv[6])
    update_timeout = int(sys.argv[7])
    sec_per_block_upper_bound = int(sys.argv[8])
    max_sum_abs_coef = {}
    ...
    parallelize(
        num_procs, f,
        (perron_polys_reg, perron_nums_reg, perron_conjs_reg, max_sum_abs_coef, blk_size, timers, debug_dir),
